// ==========================================================
// 1. إعدادات السيرفر والربط بـ Supabase وبيانات ديسبكورد
// ==========================================================
const SUPABASE_URL = 'https://jjricvhhkgvqbkgbnwtp.supabase.co'; 
const SUPABASE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Impqcmljdmhoa2d2cWJrZ2深度wtcWJrZ2Jud3RwIiwicm9sZSI6ImFub24iLCJpYXQiOjE3Nzk4OTcyNjcsImV4cCI6MjA5NTQ3MzI2N30.W-lSd89t6LHnlpUBhlM8Lkm80Rycq6THgWBUkdLBfcY'; 

// معطيات ديسكورد الخاصة بك
...
const questionsDatabase = [
  ...
  { id: 81, text: "من هو الهداف التاريخي لمنتخب فرنسا الأول؟", options: ["تييري هنري", "زين الدين زيدان", "أوليفيه جيرو", "ميشان بلاتيني"], correct: 2, minLevel: 40 },
  ...
  { id: 92, text: "من هو اللاعب الآسيوي الأعلى ترتيباً وتتويجاً بالجوائز في البريميرليغ مع توتنهام؟", options: ["بارك جي سونغ", "سون هيونغ مين", "شينجي كاغاوا", "مينامينو"], correct: 1, minLevel: 40 },
...
// المتغيرات الديناميكية للمواجهة
let gameQuestions = [];
...
// ==========================================================
// 3. إدارة تسجيل الدخول (Google & Discord) والمزامنة
// ==========================================================
...
function handleUserIn(user) {
  ...
  if (userProfileName) {
    userProfileName.innerText = user.user_metadata.full_name || user.user_metadata.name || "بطل تكتيكي";
  }
  if (userAvatarImg) {
    userAvatarImg.src = user.user_metadata.avatar_url || "https://placeholder.co/100";
  }

  // حفظ تلقائي فوري وتحديث المتصدرين
  saveUserScore(score);
}
...
if (googleLoginBtn) {
  googleLoginBtn.onclick = async function() {
    if (!supabaseClient) return alert("سيرفر السوبابيس غير متصل حالياً!");
...
// زر الحفظ اليدوي المضاف
if (manualSaveBtn) {
  manualSaveBtn.onclick = function() {
    if (!currentUser) {
      alert("⚠️ يجب عليك تسجيل الدخول أولاً لحفظ تقدمك!");
      return;
    }
...
// ==========================================================
// 4. نظام الحفظ التلقائي واليدوي وجدول الأبطال
// ==========================================================
async function saveUserScore(currentScore, isManual = false) {
  ...
  try {
    ...
    if (currentScore > previousHighScore || !data) {
      ...
      if (!upsertError) {
        loadLeaderboard();
        if (isManual) alert("💾 تم حفظ رصيدك الجديد بنجاح في لوحة الصدارة!");
      }
    } else {
...
async function loadLeaderboard() {
  if (!supabaseClient) return;
  try {
    let { data: leaderboard, error } = await supabaseClient
      .from('leaderboard')
      .select('*')
...
// ==========================================================
// 5. محرك وميكانيكية تحدي المواجهة (الأسئلة والقلوب)
// ==========================================================
function initGame() {
  score = 0;
...
function buildMatchQuestions() {
  // تصفية وعمل خلط عشوائي بالكامل للأسئلة المتاحة
  let shuffled = [...questionsDatabase].sort(() => 0.5 - Math.random());
  gameQuestions = shuffled.slice(0, 50); // صامد لـ 50 سؤالاً متتالياً
}
...
function updateStatsUI() {
  ...
  // تحديد مستوى التحدي التكتيكي الحالي
  if (difficultyTag) {
    if (totalAnsweredCount < 15) {
      difficultyTag.innerText = "📘 مستوى المبتدئين";
      difficultyTag.style.background = "#1e3a8a";
    } else if (totalAnsweredCount < 35) {
      difficultyTag.innerText = "📙 المستوى المتوسط التكتيكي";
      difficultyTag.style.background = "#b45309";
    } else {
      difficultyTag.innerText = "🟥 مستوى أساطير الـ Z3Z3";
      difficultyTag.style.background = "#7f1d1d";
    }
  }
}

function loadQuestion() {
  if (totalAnsweredCount >= 50) {
    showWinningScreen();
    return;
  }
  ...
  if (feedbackMsg) {
    feedbackMsg.innerText = "✨ فكّر جيداً قبل اختيار تكتيك الإجابة الصحيحة!";
    feedbackMsg.style.color = "#aaa";
  }
...
function checkPlayerAnswer(selectedIdx, clickedButton) {
  ...
  if (selectedIdx === currentQuestion.correct) {
    clickedButton.style.background = "#238636";
    clickedButton.style.borderColor = "#2ea043";
    if (feedbackMsg) {
      feedbackMsg.innerText = "🔥 إجابة صحيحة تكتيكية مذهلة! زاد رصيدك +2 نقاط.";
      feedbackMsg.style.color = "#2ea043";
    }
    score += 2;
    correctAnswersCount++;
    
    // حفظ تلقائي عند كل إجابة صحيحة لتحديث لوحة الصدارة
    saveUserScore(score);
  } else {
    clickedButton.style.background = "#da3637";
    clickedButton.style.borderColor = "#f85149";
    
    if (allButtons[currentQuestion.correct]) {
      allButtons[currentQuestion.correct].style.background = "#238636";
    }
    
    if (feedbackMsg) {
      feedbackMsg.innerText = "❌ إجابة خاطئة! فقدت قلباً تكتيكياً واحداً.";
      feedbackMsg.style.color = "#f85149";
    }
    hearts--;
  }
...
function showGameOverScreen() {
  ...
  if (gameOverScreen) {
    ...
    if (lostAnsweredCount) lostAnsweredCount.innerText = correctAnswersCount;
  }
  saveUserScore(score);
}
...
// ==========================================================
// 6. مستمعو الأحداث والـ Navigation الأساسية
// ==========================================================
...
// معرف الـ Client ID الخاص بك لجوجل
...
const questionsDatabase = [
  ...
  { id: 38, text: "من صاحب أطول سلسلة عدم خسارة في تاريخ الدوري الإنجليزي الممتاز؟", options: ["أرسنال", "مانشستر يونايتد", "تشيلسي", "ليفربول"], correct: 0, minLevel: 20 },
  { id: 39, text: "أي نادٍ فاز بلقب كأس العالم للأندية لعام 2023؟", options: ["ريال مدريد", "مانشستر سيتي", "تشيلسي", "فلامنغو"], correct: 1, minLevel: 20 },
  { id: 40, text: "من هو الهداف التكتيكي العبقري الذي قاد اليونان للفوز الإعجازي بـ يورو 2004؟", options: ["أوتو ريهاغل", "غوس هيدينك", "fabiocan", "كلود لو روا"], correct: 0, minLevel: 20 },
  { id: 41, text: "أي دولة فازت ببطولة كأس العالم 2014 التي أقيمت في البرازيل؟", options: ["الأرجنتين", "البرازيل", "ألمانيا", "هولندا"], correct: 2, minLevel: 20 },
  { id: 42, text: "من هو أكثر لاعب ظهوراً ومشاركة في مباريات تاريخ كأس العالم؟", options: ["ليونيل ميسي", "كريستيانو رونالدو", "لوثار ماتيوس", "جانلويجي بوفون"], correct: 0, minLevel: 20 },
  { id: 43, text: "أي نادٍ إيطالي عريق وعالمي يُلقب بـ 'السيدة العجوز'؟", options: ["ميلان", "إنتر ميلان", "يوفنتوس", "روما"], correct: 2, minLevel: 20 },
  { id: 44, text: "من هو اللاعب الأفريقي الوحيد في التاريخ الذي فاز بجائزة الكرة الذهبية؟", options: ["صامويل إيتو", "ديديه دروغبا", "جورج ويا", "محمد صلاح"], correct: 2, minLevel: 20 },
  ...
  { id: 90, text: "من هو الحكم الإيطالي الأسطوري الشهير والأصلع الذي أدار نهائي كأس العالم 2002؟", options: ["بييرلويجي كولينا", "هاوارد ويب", "مارك كلاتنبرغ", "نستور بيتانا"], correct: 0, minLevel: 40 },
  { id: 91, text: "أي نادٍ إنجليزي يلقب بـ 'التوفيز' ويقع في مدينة ليفربول؟", options: ["ليفربول", "إيفرتون", "ترانمير روفرز", "أستون فيلا"], correct: 1, minLevel: 40 },
  { id: 92, text: "من هو اللاعب الآسيوي الأعلى ترتيباً وتتويجاً بالجوائز في البريميرليغ مع توتنهام؟", options: ["PARK", "سون هيونغ مين", "شينجي كاغاوا", "مينامينو"], correct: 1, minLevel: 40 },
  { id: 93, text: "من هو الهداف التاريخي لمنتخب البرازيل في المباريات الرسمية للفيفا؟", options: ["بيليه", "رونالدو نازاريو", "نيمار داسيلفا", "روماريو"], correct: 2, minLevel: 40 },
  { id: 94, text: "أي منتخب فاز بكأس العالم لكرة القدم عام 2010 في جنوب أفريقيا؟", options: ["هولندا", "ألمانيا", "إسبانيا", "الأوروغواي"], correct: 2, minLevel: 40 },
  { id: 95, text: "من هو المدافع التاريخي الوحيد الذي فاز بالكرة الذهبية في القرن الحالي (2006)？", options: ["باولو مالديني", "فابيو كانافارو", "سيرجيو راموس", "جون تيري"], correct: 1, minLevel: 40 },
  { id: 96, text: "ما هو لقب نادي نيوكاسل يونايتد الإنجليزي؟", options: ["الماكبايس (القماري)", "الثعالب", "الذئاب", "الفيلانز"], correct: 0, minLevel: 40 },
...
// ==========================================================
// 3. إدارة تسجيل الدخول (Google SDK & Discord) والمزامنة
// ==========================================================
window.onload = function() {
  initAuthCheck();
  initGoogleAuth(); // تشغيل تهيئة زر جوجل المدمج
  initGame();
  loadLeaderboard();
};
...
function initGoogleAuth() {
  if (typeof google !== 'undefined') {
    // تهيئة نظام جوجل الـ Client ID الخاص بك
    google.accounts.id.initialize({
      client_id: GOOGLE_CLIENT_ID,
      callback: handleGoogleCredentialResponse
    });

    // رسم زر جوجل داخل الـ div المخصص له بستايل متناسق مع اللون الداكن للموقع
    google.accounts.id.renderButton(
      document.getElementById("buttonDiv"),
      { theme: "filled_blue", size: "medium", width: 220 }
    );
  }
}

// دالة فك التوكن واستخراج بروفايل جوجل
function handleGoogleCredentialResponse(response) {
  try {
    ...
    // محاكاة كائن بيانات المستخدم ليتوافق مع بنية الموقع الحالية
    const mockUser = {
    ...
    handleUserIn(mockUser);
    alert("🎉 أهلاً بك يا " + googleUser.name + "! تم تسجيل الدخول عبر Google بنجاح.");
  } catch(err) {
...
function handleUserIn(user) {
  currentUser = user;
  if (guestModeMsg) guestModeMsg.style.display = "none";
  // إخفاء الـ Container الكامل للزر عند الدخول
...
if (logoutBtn) {
  logoutBtn.onclick = async function() {
    if (supabaseClient && !currentUser.id.startsWith("google_")) {
...
function buildMatchQuestions() {
  let shuffled = [...questionsDatabase].sort(() => 0.5 - Math.random());
  gameQuestions = shuffled.slice(0, 50); 
}
...
if (menuReset) {
  menuReset.onclick = function(e) { e.preventDefault(); initGame(); menuDropdown.classList.add("hidden"); };
}

const menuInfo = document.getElementById("menuInfo");
if (menuInfo) {
  menuInfo.onclick = function(e) { e.preventDefault(); alert("🏆 كأس المعرفة:\n• نظام متصدرين لحساب عدد الأسئلة الصحيحة مدمج بالكامل!"); menuDropdown.classList.add("hidden"); };
}

document.querySelectorAll(".restart-action-btn").forEach(btn => {
